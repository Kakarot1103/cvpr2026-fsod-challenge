...
def evaluate_dataset(args, model, processor, dataset_path,
    ...
    readme_json_path = f"{args.data_instr_path}_{dataset_basename}.json"
    if not os.path.isfile(readme_json_path):
        instr_dir = os.path.dirname(args.data_instr_path) if os.path.dirname(args.data_instr_path) else "."
        instr_prefix = os.path.basename(args.data_instr_path) + "_"
        best_match = None
        best_key_len = 0
        for f in os.listdir(instr_dir):
            if f.startswith(instr_prefix) and f.endswith(".json"):
                key = f[len(instr_prefix):-5]
                if dataset_basename.startswith(key) and len(key) > best_key_len:
                    best_match = os.path.join(instr_dir, f)
                    best_key_len = len(key)
        if best_match:
            readme_json_path = best_match
    ...
    if all(os.path.isfile(p) for p in prediction_cache_paths.values()) and \
        ...
        for img_info in tqdm(images_to_process, desc=f"Processing images in {os.path.basename(dataset_path)}"):
            ...
            yield {
                ...
                "pred_bboxes": [det["bbox"] for det in (all_detections["ranking"] or [])],
...
def run_single_dataset_evaluation(args, model=None, processor=None):
    ...
    if model is None or processor is None:
        model, processor = utils.load_qwen_model(args.model_name, server_url=args.server_url, served_model_name=args.served_model_name)
    else:
