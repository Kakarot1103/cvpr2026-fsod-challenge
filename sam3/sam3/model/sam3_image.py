...
class Sam3Image(torch.nn.Module):
    ...
    def _encode_prompt(
        ...
        # Encode geometry
        if backbone_out.get('support_info') is None:
            backbone_out['support_info'] = {'store':False,'use': False}
        geo_feats, geo_masks = self.geometry_encoder(
            geo_prompt=geometric_prompt,
            img_feats=img_feats,
            img_sizes=vis_feat_sizes,
            img_pos_embeds=img_pos_embeds,
            support_info = backbone_out['support_info'] 
        )
        ...
        # if support_info['use']:
        #     geo_feats = support_info['geo_feats'].clone()
        #     geo_masks = support_info['geo_masks'].clone()
        if encode_text:
        ...
        # backbone_out['support_info']['geo_feats'] = geo_feats.clone()
        # backbone_out['support_info']['geo_masks'] = geo_masks.clone()
    ...
    def forward_grounding(
        ...
        with torch.profiler.record_function("SAM3Image._encode_prompt"):
            prompt, prompt_mask, backbone_out = self._encode_prompt(
                backbone_out, find_input, geometric_prompt,support_info=support_info
            )
        # Run the encoder
