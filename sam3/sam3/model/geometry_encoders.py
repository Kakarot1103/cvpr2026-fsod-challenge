...
class SequenceGeometryEncoder(nn.Module):
    ...
    def _encode_boxes(self, boxes, boxes_mask, boxes_labels, img_feats,ref_mask=None):
        boxes_embed = None
        ...
        # if False:
            ...
            if ref_mask is not None:
                img_feats_masked = img_feats * ref_mask.unsqueeze(0).unsqueeze(0)
            else:
                img_feats_masked = img_feats
            sampled = torchvision.ops.roi_align( 
               img_feats_masked, boxes_xyxy.float().transpose(0, 1).unbind(0), self.roi_size
            )
    ...
    def forward(self, geo_prompt: Prompt, img_feats, img_sizes, img_pos_embeds=None, support_info=None):
        ...
        if support_info['use']:
            final_embeds = support_info['geo_feats'].clone()
            final_mask = support_info['geo_mask'].clone()
        ...
        if support_info['store']:
            support_info['geo_feats'] = final_embeds.clone()
            support_info['geo_mask'] = final_mask.clone()
